// BUDGET VISION — Full App v6
// Theme · Login · Daily Expenses · Auto-categorize · Charts
...
let income = 0, expenses = {}, chart = null, dailyLogs = [], selectedCatOverride = null, loginPhoto = null;
...
function handlePhotoSelect(input){
  ...
  r.onload = e => {
    loginPhoto = e.target.result;
    document.getElementById('login-photo-preview').innerHTML = `<img src="${loginPhoto}" />`;
  };
...
function handleLogin(){
  ...
  localStorage.setItem('bv_user_name', name);
  if(loginPhoto) localStorage.setItem('bv_user_photo', loginPhoto);
  localStorage.setItem('bv_logged_in','1');
  showApp(name, loginPhoto);
}

function showApp(name, photo){
  document.getElementById('login-screen').classList.add('hidden');
  document.getElementById('main-app').classList.remove('hidden');
  document.getElementById('topbar-username').textContent = name;
  document.getElementById('profile-name-display').textContent = name;
  document.getElementById('name-edit-input').value = name;
  const initials = name.charAt(0).toUpperCase();
  const avatarEl = document.getElementById('topbar-avatar');
  const profileEl = document.getElementById('profile-photo-display');
  if(photo){
    avatarEl.innerHTML = `<img src="${photo}" alt="${name}" />`;
    profileEl.innerHTML = `<img src="${photo}" alt="${name}" />`;
  } else {
...
function handleLogout(){ localStorage.removeItem('bv_logged_in'); location.reload(); }

function checkAuth(){
  if(localStorage.getItem('bv_logged_in')){
    showApp(localStorage.getItem('bv_user_name')||'User', localStorage.getItem('bv_user_photo')||null);
  }
}
...
function handleProfilePhotoChange(input){
  ...
  r.onload = e => {
    ...
    localStorage.setItem('bv_user_photo', src);
    document.getElementById('profile-photo-display').innerHTML = `<img src="${src}" />`;
    document.getElementById('topbar-avatar').innerHTML = `<img src="${src}" />`;
  };
  r.readAsDataURL(file);
}

// NAME EDIT
function toggleNameEdit(){ document.getElementById('name-edit-row').classList.toggle('hidden'); }
function saveName(){
  const name = document.getElementById('name-edit-input').value.trim(); if(!name) return;
  localStorage.setItem('bv_user_name', name);
  document.getElementById('profile-name-display').textContent = name;
  document.getElementById('topbar-username').textContent = name;
  const av = document.getElementById('topbar-avatar');
  if(!av.querySelector('img')) av.textContent = name.charAt(0).toUpperCase();
...
function saveDailyLogs(){ localStorage.setItem('bv_daily_logs', JSON.stringify(dailyLogs)); }

function loadDailyLogs(){
  const saved = localStorage.getItem('bv_daily_logs');
  if(saved){
    dailyLogs = JSON.parse(saved);
    const totals = {};
    dailyLogs.forEach(e => { totals[e.catId] = (totals[e.catId]||0) + e.amount; });
  ...
  document.getElementById('categories-list').innerHTML = CATEGORIES.map(cat=>`
    <div class="cat-item">
      ...
      <div class="cat-input-wrap">
        ...
        <input type="number" class="cat-input" id="inp-${cat.id}" placeholder="0" min="0" inputmode="numeric" data-id="${cat.id}" />
      </div>
    </div>`).join('');
...
function updateHero(total){
  const rem = income-total; const sr = income>0?Math.max((rem/income)*100,0):0;
...
function updateInsights(total){
  ...
  if(rem<0) ins.push({t:'danger',i:'🚨',m:`Overspending by ${fmtFull(Math.abs(rem))}! Cut expenses now.`});
  ...
  else if(sr>0) ins.push({t:'warning',i:'💡',m:`Only ${sr.toFixed(0)}% saved. Aim for 20% (${fmtFull(income*0.20)}/mo).`});
  const hp=income>0&&expenses.housing?(expenses.housing/income)*100:0;
  if(hp>40) ins.push({t:'warning',i:'🏠',m:`Rent is ${hp.toFixed(0)}% of income. Keep it under 30%.`});
...
function updateRule(total){
  ...
  const ok=np<=52&&wp<=32&&sp>=18;
  document.getElementById('rule-status').textContent=income>0?(ok?'✅ On track':'⚠️ Needs work'):'—';
...
document.getElementById('income-input').addEventListener('input', e=>{ income=parseFloat(e.target.value)||0; update(); });
